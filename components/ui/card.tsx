...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "bg-card text-card-foreground group/card flex flex-col gap-5 overflow-hidden rounded-2xl border border-border/60 py-5 shadow-romance has-[>img:first-child]:pt-0 data-[size=sm]:gap-4 data-[size=sm]:py-4 text-sm *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-2xl px-5 group-data-[size=sm]/card:px-4 [.border-t]:pt-5 group-data-[size=sm]/card:[.border-t]:pt-4",
        className
      )}
      {...props}
    />
